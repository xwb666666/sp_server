...
import cn.stylefeng.guns.cloud.auth.modular.ssomgr.util.GatewayActuatorUtil;
...
public class GatewayActuatorServiceImpl implements GatewayActuatorService {
    ...
    public List<Map<String, Object>> routeList(String routeId) {
        ...
        //返回指定路由信息
        if (ObjectUtil.isNotEmpty(routeId) && ObjectUtil.isNotEmpty(resultList)) {
    ...
    private void checkRouteInfo(String routeInfo) {
        ...
        //校验filters
        if (ObjectUtil.isNotEmpty(jsonObject.get("filters"))) {
    ...
    private void checkKeys(JSONObject jsonObject) {
        ...
        if (!(jsonObject.containsKey("predicates") && ObjectUtil.isNotEmpty(jsonObject.get("predicates")))) {
        ...
        if (!(jsonObject.containsKey("uri") && ObjectUtil.isNotEmpty(jsonObject.get("uri")))) {
            throw new ServiceException(400, "uri属性不能为空");
        }
        if (!(jsonObject.containsKey("order") && ObjectUtil.isNotEmpty(jsonObject.get("order")))) {

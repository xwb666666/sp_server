 ...
 */
package cn.stylefeng.guns.cloud.libs.cloud.feign;
...
public class GunsFeignErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(String methodKey, Response response) {
        ...
        // 1.首先解析出来ResponseBody内容，如果body解析异常则直接抛出ServiceException
        String resposeBody;
        try {
        ...
        // 4.首先判断是否有exceptionClazz字段，如果有，代表是服务抛出的业务接口异常ApiServiceException的子类，那么需要返回这个异常
        ...
        // 5.如果不是ApiServiceException的子类，则抛出ServiceException
        if (message == null) {
    ...
    private ApiServiceException getApiServiceExceptionByClassName(String className, Integer code, String message) {
        ...
        try {
            ...
            Constructor constructor = clazz.getConstructor(AbstractBaseException.class);
            return (ApiServiceException) constructor.newInstance(new AbstractBaseException() {
                @Override

...
import cn.stylefeng.guns.cloud.system.modular.res.enums.AppExceptionEnum;
...
import cn.stylefeng.guns.cloud.system.modular.sys.enums.MenuExceptionEnum;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public List<MenuNode> getLeftMenuList(Long appId, Long accountId) {
        if (log.isDebugEnabled()) {
            log.debug("开始获取左侧菜单---appId：" + appId + "---accountId:" + accountId);
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("获取到appCode---appCode：" + app.getAppCode());
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("获取到用户的资源---resourceIds：" + JSON.toJSONString(resCodes));
        }
        ...
        if (log.isDebugEnabled()) {
            log.debug("获取资源对应的菜单---menus：" + JSON.toJSONString(menus));
        }
    ...
    private MenuNode buildMenuNode(SysMenu menu) {
        ...
        return new MenuNode(
                ...
                menu.getOrderNo(), menu.getUrl(), menu.getIcon(), menu.getHiddenFlag(), menu.getMenuTips(),
    ...
    private boolean childrenIsAllHidden(List<MenuNode> menuNods) {
        ...
        for (MenuNode node : menuNods) {
            if ("N".equals(node.getHiddenFlag())) {
                return false;
            }
        }
